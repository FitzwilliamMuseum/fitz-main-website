<div class="exhibition-hero">
    <div class="exhibition-hero-inner">
        ...
        <div class="hero-text">
            @if (Request::is('plan-your-visit/exhibitions/journeys-with-mai'))
                <h1>Tau o Mai | Journeys with Mai</h1>
            @else
                <h1>{{ $exhibition['exhibition_title'] }}</h1>
            @endif
            @if(isset($exhibition['display_dates']) && $exhibition['display_dates'])
                <div class="hero__exhibition-date">
                    ...
                    <p>{{ Carbon\Carbon::parse($exhibition['exhibition_start_date'])->format('j F Y') }}
                        @if($exhibition['display_end_date'])
