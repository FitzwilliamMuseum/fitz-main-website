                ...
                <div class="col-md-4 mb-3">
                    <div class="card h-100">
                        @if(strpos($result['contentType'][0], 'instagram') !== false)
                            <blockquote class="instagram-media" data-instgrm-permalink="{{ $result['url'][0] }}"
                                        data-instgrm-version="13"
                                        style=" background:#FFF; border:0; border-radius:3px; box-shadow:0 0 1px 0 rgba(0,0,0,0.5),0 1px 10px 0 rgba(0,0,0,0.15); margin: 1px; max-width:540px; min-width:326px; padding:0; width:99.375%; width:-webkit-calc(100% - 2px); width:calc(100% - 2px);">
                                <div style="padding:16px;"><a
                                        href="{{ $result['url'][0] }}?utm_source=ig_embed&amp;utm_campaign=loading"
                                    ...
                                    <p style=" color:#c9c8cd; font-family:Arial,sans-serif; font-size:14px; line-height:17px; margin-bottom:0; margin-top:8px; overflow:hidden; padding:8px 0 7px; text-align:center; text-overflow:ellipsis; white-space:nowrap;">
                                        <a href="{{ $result['url'][0] }}/?utm_source=ig_embed&amp;utm_campaign=loading"
                                           style=" color:#c9c8cd; font-family:Arial,sans-serif; font-size:14px; font-style:normal; font-weight:normal; line-height:17px; text-decoration:none;"
                                           target="_blank">A post shared by The Fitzwilliam Museum (@fitzmuseum_uk)</a>
                                    </p></div>
                            </blockquote>
                            <script async src="//www.instagram.com/embed.js"></script>
                        @elseif(strpos($result['contentType'][0], 'twitter') !== false)
                            <blockquote class="twitter-tweet"><a href="{{$result['url'][0]}}"></a></blockquote>
                            <script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script>
                        @elseif(strpos($result['contentType'][0], 'schools') !== false)
                            <a href="{{ $result['url'][0]}}"><img
                                    src="https://content.fitz.ms/fitz-website/assets/portico.jpg"
                                    class="img-fluid responsive-item" alt="FitzVirtual Logo" loading="lazy"/></a>
                        @elseif(isset($result['searchImage']))
                            <div class="embed-responsive embed-responsive-1by1">
                                <a href="{{ $result['url'][0]}}"><img src="{{$result['searchImage'][0]}}"
                                                                      class="img-fluid embed-responsive-item"
                                                                      alt="Fitzwilliam Museum logo"
                                                                      loading="lazy"/></a>
                            </div>
                        @else
                            <a href="{{ $result['url'][0]}}"><img
                        ...
                        <div class="card-body ">
                            ...
                            @if(isset($result['mimetype']))
                                ...
                                @else
                                    <p>
                                        <a href="{{URL::to('/')}}/{{$result['url'][0]}}">{{URL::to('/')}}
                                            /{{$result['url'][0]}}</a>
                                    </p>
                                @endif
                            @endif
                          ...
                          <i class="fa fa-download mr-2" aria-hidden="true"></i> @humansize($result['filesize'][0],2)
                                    ...
                                    @if($result['contentType'][0] == 'learning_files')
                                        ...
                                        @if(isset($result['keystages']))
                                            <span
                                                class="badge badge-wine p-2">Key Stages this is for: {{ implode(', ', $result['keystages']) }}</span>
                                        @endif
