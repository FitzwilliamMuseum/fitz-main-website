    ...
    <div class="row ">
        ...
        <div class="col-md-7 mb-3">
            <div class="shadow-sm p-3 mb-3 mt-3">
                <figure class="figure">
                    <img src="{{ $ltd['focus_image']['data']['full_url'] }}"
                         class="img-fluid" alt="{{ $ltd['title_of_work'] }}" loading="lazy"
                         width="{{ $ltd['focus_image']['width'] }}"
                        ...
                        @if(!is_null($ltd['associated_pharos'][0]['pharos_id']['image']))
                            <a href="{{ route('highlight', $ltd['associated_pharos'][0]['pharos_id']['slug']) }}">
                                <img class="img-fluid"
                                     ...
                                     alt="{{ $ltd['associated_pharos'][0]['pharos_id']['image_alt_text'] }}"
                                     loading="lazy"
                                     width="{{ $ltd['associated_pharos'][0]['pharos_id']['image']['data']['thumbnails'][4]['width'] }}"
                                     height="{{ $ltd['associated_pharos'][0]['pharos_id']['image']['data']['thumbnails'][4]['height'] }}"
                                /></a>
                        @endif
                        <div class="card-body h-100">
                            <div class="contents-label mb-3">
                                <h3>
                                    <a href="{{ route('highlight', $ltd['associated_pharos'][0]['pharos_id']['slug']) }}">{{ $ltd['associated_pharos'][0]['pharos_id']['title']}}</a>
                                </h3>
                            </div>
                        </div>
                    </div>
        ...
        <div class="col-12 shadow-sm p-3 mx-auto mb-3 ">
            <div class="ratio ratio-16x9">
                <iframe title="A YouTube video related to {{ $ltd['title_of_work'] }}"
                        src="https://www.youtube.com/embed/{{ $ltd['youtube_id'] }}"
        ...
        <div class="col-12 shadow-sm p-3 mx-auto mb-3 ">
            <div class="ratio ratio-16x9">
                <iframe title="A Vimeo video related to {{ $ltd['title_of_work'] }}"
                        src="https://player.vimeo.com/video/{{$ltd['vimeo_id']}}"
                        allowfullscreen></iframe>
            </div>
        </div>
    @endif
