        ...
        <div class="card h-100">
          @if(!empty($instagram['hero_image']))
            <a href="{{ route('instagram.story', $instagram['slug']) }}"><img class="img-fluid" src="{{ $instagram['hero_image']['data']['thumbnails'][4]['url']}}"
              ...
              loading="lazy"/></a>
            @else
              <a href="{{ route('instagram.story', $instagram['slug']) }}"><img class="img-fluid" src="https://content.fitz.ms/fitz-website/assets/gallery3_roof.jpg?key=directus-large-crop"
                alt="A stand in image for {{ $instagram['title'] }}"/></a>
              @endif
