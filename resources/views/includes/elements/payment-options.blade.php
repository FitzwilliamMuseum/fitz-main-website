<div class="container mt-4 p-5">
    <div class="card mt-4 col-md-12">
        <div class="row mb-4 no-gutters">
            <h3 class="lead p-heading p-heading--single">Friends individual</h3>
            <hr /> 
            <div class="col-md-4 border d-flex rounded text-center p-container p-container--single">
                <span class="lead">Friends individual</span>
                <figure class="fluid">
                    <img class="img-fluid p-image p-image--single" src="data:image/svg+xml;base64," alt="user icon" />  
                </figure>
  
            </div>
            <div class="col-md-8 mt-4">
                <div class="card h-100 justify-content-center">
                    <a href="https://tickets.museums.cam.ac.uk/donate/contribute1?ct=2" class="align-self-center w-50 btn btn-info btn-credit">Credit Card ￡55</a>
                    <a href="https://fitz-tnew-test.tnhs.cloud/components/precart?exec=true&procedureId=54&contrib_type=2&is_dd=y" class="align-self-center w-50 mt-3 btn btn-hockney btn-direct">Direct Debit ￡77</a>
                </div>
            </div>
        </div>
        <div class="mt-4 row mb-4 no-gutters">
            <h3 class="lead p-heading p-heading--joint">Joint Friends</h3>
            <hr />
            <div class="col-md-4 border d-flex rounded text-center p-container p-container--joint">
                <span class="lead">Joint Friends</span>
                <figure class="fluid">
                    <img class="img-fluid p-image p-image--joint" src="data:image/svg+xml;base64," alt="users icons" />
                </figure>
            </div>
            <div class="col-md-8 mt-4">
                <div class="card h-100 justify-content-center">
                    <a href="https://tickets.museums.cam.ac.uk/donate/contribute1?ct=3" class="align-self-center w-50 btn btn-info btn-credit">Credit Card ￡55</a>
                    <a href="https://fitz-tnew-test.tnhs.cloud/components/precart?exec=true&procedureId=54&contrib_type=3&is_dd=y" class=" align-self-center w-50 mt-3 btn btn-hockney btn-direct">Direct Debit ￡77</a>
                </div>
            </div>
        </div>
    </div>
</div>